...
#define SGL_DEF_COLOUR(name) static const Colour& name

namespace sgl
{
	using PackedColour = uint32;

    struct SGL_API Colour final
    {
        float r, g, b, a;

        static PackedColour Pack(const Colour& colour);
        static Colour Unpack(PackedColour packedColour);

        Colour() : r(0), g(0), b(0), a(1) {}
        Colour(float r, float g, float b, float a = 1) : r(r), g(g), b(b), a(a) {}
        Colour(uint8 r, uint8 g, uint8 b, uint8 a = 255) : r(r / 255.0f), g(g / 255.0f), b(b / 255.0f), a(a / 255.0f) {}
        Colour(uint32 r, uint32 g, uint32 b, uint32 a = 255) : r(r / 255.0f), g(g / 255.0f), b(b / 255.0f), a(a / 255.0f) {}
